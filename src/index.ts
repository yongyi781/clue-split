...
const clueCompleteRegex =
  /Congratulations! You have now completed [\d,.]+ (?<clueType>\w+) treasure trails./;
  ...
  setup() {
    ...
    function init() {
      ...
      setInterval(() => {
        ...
        currentClueTime.value = Math.round(
          (Date.now() - lastClueTime.getTime()) / 1000
        );
      }, 500);
    }
    ...
    function captureChat() {
      ...
      for (let line of lines) {
        ...
        if (match != null && match.length > 0) {
          let timestamp = chatTimestampToDateTime(
            ChatBoxReader.getMessageTime(line.text)
          );
    ...
    function exportCsv() {
      const rows = [["#", "Elapsed", "Clue time", "Clues/hr"]].concat(
        getTable(true).map((x) => [
          x.id.toString(),
          x.elapsed,
          x.duration,
      ...
      link.setAttribute(
        "download",
        `clue_split_${clueType.value ?? "unknown"}_${formatDateTimeFileName(
          startTime.value
        )}.csv`
      );
